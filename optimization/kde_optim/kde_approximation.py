...
if __name__ == '__main__':
    ...
    for h, w, c in tqdm(itertools.product(range(H), range(W), range(C))):
        ...
        optim_kde_approximation = factory.get_kde_approximation(
            "lbfgs")(copy.deepcopy(pool_points),copy.deepcopy(pool_weights))
        cluster_time = 0
        optim_time = 0

        append_points = activation_arrays[1, :, h, w, c].tolist()
        ...
        all_points = activation_arrays[:2, :, h, w, c].reshape(1, -1)[0]
